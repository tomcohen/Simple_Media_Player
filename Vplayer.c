...
// global variables
int global_videoIndex = 0;
int global_audioIndex = 0;
SDL_Surface *screen[MAX_CHANNELS];		// global array of screens - size = number of channels
VideoState *global_video_state[MAX_CHANNELS];	// global array of video state - size = number of channels
SDL_mutex *global_mutex_lock;
SDL_cond *global_cond;
AVPacket f_pkt;
...
int packet_queue_put(PacketQueue *q, AVPacket *pkt){
	...
	if ((av_dup_packet(pkt) < 0) && (pkt != &f_pkt)){
		return -1;
	}
...
static void packet_queue_f (PacketQueue *q){
	AVPacketList *pkt, *pkt2;
	SDL_LockMutex(q->mutex);
...
static int packet_queue_get(PacketQueue *q, AVPacket *pkt, int block, VideoState* is){
	AVPacketList *pkt1;
	int ret;
	SDL_LockMutex(q->mutex);
...
int audio_decode_frame(VideoState *is, double *pts_ptr){
	...
	for (;;){
		...
		/* next packet */
		if (packet_queue_get(&is->audioq, pkt, 1, is) < 0){
			return -1;
		}
		if (pkt->data == f_pkt.data){
...
void video_display(VideoState *is){
	...
	if (vp->bmp){
		...
		h = screen[is->videoIndex]->h;
		w = ((int) rint(h * aspect_ratio)) & -3;
		if (w > screen[is->videoIndex]->w){
			w = screen[is->videoIndex]->w;
			h = ((int) rint(w / aspect_ratio)) & -3;
		}
		x = (screen[is->videoIndex]->w - w) / 2;
		y = (screen[is->videoIndex]->h - h) / 2;
		...
		rect.h = h;
		SDL_LockMutex(global_mutex_lock);
		if(is->videoIndex == global_videoIndex){
...
int stream_component_open(VideoState *is, int stream_index){
	...
	if (codecCtx->codec_type == AVMEDIA_TYPE_AUDIO){
		...
		is->want_spec = wanted_spec; 	// check it!
		SDL_LockMutex(global_mutex_lock);
		if(is->videoIndex == global_videoIndex){
	...
	if (!codec || (avcodec_open2(codecCtx, codec, NULL) < 0)){ //&optionDict
...
void change_audchannel (int next){
	SDL_LockMutex(global_mutex_lock);
	global_audioIndex = next -1;
	SDL_UnlockMutex(global_mutex_lock);
}

void change_vidchannel (int next){
	SDL_LockMutex(global_mutex_lock);
	global_videoIndex = next -1;
	SDL_UnlockMutex(global_mutex_lock);
}

void change_channel (int next){
	SDL_LockMutex(global_mutex_lock);
	global_audioIndex = next -1;
	global_videoIndex = next -1;
	SDL_UnlockMutex(global_mutex_lock);
